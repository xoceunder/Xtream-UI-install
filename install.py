...
def prepare(rType="MAIN"):
    ...
    if rType == "MAIN":
        ...
        #    os.system("sudo add-apt-repository -y 'deb [arch=amd64,arm64,ppc64el] http://ams2.mirrors.digitalocean.com/mariadb/repo/11.5/ubuntu %s main' > /dev/null 2>&1" % rVersions[rVersion])
    ...
    #subprocess.run("debconf-set-selections <<< 'iptables-persistent iptables-persistent/autosave_v4 boolean true' > /dev/null 2>&1", shell=True)
    #subprocess.run("debconf-set-selections <<< 'iptables-persistent iptables-persistent/autosave_v6 boolean true' > /dev/null 2>&1", shell=True)
    for rPackage in rPackages:
        if not is_installed(rPackage):
            printc("Installing %s" % rPackage)
            #os.system("sudo apt-get -y install %s > /dev/null" % rPackage)
